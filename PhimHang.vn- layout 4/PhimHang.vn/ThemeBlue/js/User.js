...
function viewModel() {
    ...
    self.init = function () {
        ...
        $.ajax({
            cache: false,
            type: "GET",
            url: '/User/GetPostMoreByUserId',
            data: { userid: $('#HiddentUserId').val(), skipposition: 0, filter: "ALL" },
    ...
    self.addReply = function () { // them tra loi
        ...
        commenthub.server.addReply({ "Message": self.newReply(), "PostedBy": postidCurrent }, 'USER', $('#HiddentCureentUserId').val(), $('#HiddentUserName').val(), $('#HiddentAvataEmage').val())
               .done(function () {
    ...
    self.FilterAll = function (stringFilter) {
        if (checkLoadFirst == 1) {
            ...
            $.ajax({
                ...
                type: "GET",
                url: '/User/GetPostMoreByUserId',
                data: { userid: $('#HiddentUserId').val(), skipposition: 0, filter: filterhere },
                beforeSend: function (xhr) {
                    //Add your image loader here
                    //$('.ajaxLoadingImage').html('<img src="/images/ajax-loader_cungphim.gif" />');
                },
    ...
    $(window).scroll(function () { // scroll endpage load more       
        if (document.documentElement.clientHeight + $(document).scrollTop() >= $( document ).height() - 100 && checkLoadFirst == 1 && loadSlow == 'Y') {
            ...
            $.ajax({
                ...
                type: "GET",
                url: '/User/GetPostMoreByUserId',
                data: { userid: $('#HiddentUserId').val(), skipposition: self.posts().length, filter: filterhere },
                beforeSend: function (xhr) {
                    //Add your image loader here
